...
public class BinarySearchTreeIterator {
   ...
   * 
   * 2. Complexity 
   * - Time O(H) for constructor, average O(1) for next and O(1) for hasNext
   * - Space O(H)
   * 
   * @param root
   */
  public BinarySearchTreeIterator(TreeNode root) {
    stack = new Stack<>();
    traverseLeftBottom(root);
  }

  public int next() {
    TreeNode next = stack.pop();
    traverseLeftBottom(next.right);
    return next.val;
  }

  private void traverseLeftBottom(TreeNode root) {
    while (root != null) {
      stack.push(root);
      root = root.left;
    }
