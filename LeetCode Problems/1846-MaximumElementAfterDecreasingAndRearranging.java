...
public class MaximumElementAfterDecreasingAndRearranging {
  ...
  /**
   * 1. Approach
   * Sort Array and One Pass. Once the array is sorted, it could guarantee that .... ai, aj ..... (ai <= aj).
   * Then the maxElement[j] will either be previous maxElement[i] + 1 (if maxElement[i] + 1 < aj) otherwise aj itself
   * or in other word maxElement[i].
   * 
   * 2. Complexity 
   * - Time O(NlogN) for array sorting
   * - Space O(1)
   * 
   * @param arr
   * @return
  ...
  public int maximumElementAfterDecrementingAndRearranging(int[] arr) {
    ...
    for (int a : arr) {
      maxElement = Math.min(maxElement + 1, a);
    }
