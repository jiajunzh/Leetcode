...
public class IntersectionOfTwoLinkedLists {
    ...
    /**
     * 1. Approach
     * Two Pointers (Count Size and Then Skip Nodes).
     *    -- a ------
     *               --------- c --------
     * ----- b ------
     *
     * Once the first (b - a) nodes are skipped, the second list is at the same start point as the first list
     *
     * 2. Complexity
     * - Time O(N + M)
     * - Space O(1)
    ...
    public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
        ...
        ListNode newHeadA = skipNode(headA, sizeA - sizeB);
        ListNode newHeadB = skipNode(headB, sizeB - sizeA);
    ...
    private ListNode skipNode(ListNode head, int skipCount) {
        while (skipCount > 0) {
            head = head.next;
            skipCount--;
        }
