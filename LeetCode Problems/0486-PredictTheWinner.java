 ...
 * each turn, the player takes one of the numbers from either end of the array (i.e., nums[0] or nums[nums.length - 1])
  ...
  public boolean PredictTheWinner(int[] nums) {
    ...
    for (int size = 2; size <= n; size++) {
      for (int i = 0; i + size <= n; i++) {
        dp[i] = Math.max(nums[i] - dp[i + 1], nums[i + size - 1] - dp[i]);
      }
    }
    return dp[0] >= 0;
  }
