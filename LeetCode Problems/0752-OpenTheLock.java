...
public class OpenTheLock {
   ...
   * 
   * 2. Complexity 
   * - Time O(N^2 * A^N + D) where N is the number of digits in lock and A is number of alphabets and D is number of deadends
   * - Space O(A^N + D)
   * 
   * @param deadends
   * @param target
  ...
  public int openLock(String[] deadends, String target) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (target.equals(curr)) return turn;
        for (int index = 0; index < 4; index++) {
          final int currDigit = curr.charAt(index) - '0';
          for (int delta = 9; delta <= 11; delta += 2) {
            final int nextDigit = (currDigit + delta) % 10;
            final String next = curr.substring(0, index) + nextDigit + curr.substring(index + 1, 4);
